...
├── 📄 발표_가이드_수정본.md                   # 10분 발표용 가이드
├── 📓 서울시_편의점_매출_통합분석_최종본.ipynb  # 메인 분석 노트북
├── 📊 분석데이터.csv                          # 전처리 완료된 분석 데이터
└── 📂 outputs/                               # 실행 시 생성되는 결과물
    ├── 01_매출분포.png
    ├── 02_이상치분석_박스플롯.png
    ├── 03_분기별_매출추이.png
    ├── 04_H1_유동인구_매출.png
    ├── ...
    ├── 10_표준화_회귀계수.png                  # 신규 추가
    └── 모델성능_비교.csv
```
...
### 데이터 검증
- **이상치 분석 (IQR)**: 매출/점포수/유동인구의 이상치 현황 파악 → 제거 없이 진행
- **결측치 처리**: 상권유형 미분류 지역은 '미분류'로 처리
- **데이터 정합성**: 연도별/분기별 데이터 건수 균등 분포 확인
- **데이터 품질**: 6,097건, 결측치 0건

### 탐색적 분석 (EDA)
- **분기별 매출 추이**: 2022Q1~2025Q3 시계열 트렌드 분석
- **상권유형별 추이**: 상권별 성장/하락 패턴 비교
- **상관분석 & 시각화**: 변수 간 관계 파악
...
### 사용 기법
- **데이터 검증**: IQR 이상치 분석, 결측치 확인, 정합성 검증
...
### 3. 실행
1. 모든 파일을 **같은 폴더**에 저장
2. Jupyter Notebook 또는 JupyterLab 실행
3. `서울시_편의점_매출_통합분석_최종본.ipynb` 파일 열기
4. **Kernel → Restart & Run All** 실행

### 4. 예상 실행 시간
- 약 1~2분 (머신러닝 모델 학습 포함)
